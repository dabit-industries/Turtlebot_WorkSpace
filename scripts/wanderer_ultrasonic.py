...
import rospy
from kobuki_msgs.msg import SensorState, Sound, BumperEvent, ButtonEvent, SensorState
...
class wanderer:
	...
	self.sect_3 = 0
        self.bumped = False
        self.stopped = False
        self.stopped_time = 2 #seconds
        self.stopped_until = rospy.Time.now()
    ...
    def listener(self):
        ...
        self.sub_sensor_state = rospy.Subscriber('/mobile_base/sensors/core', SensorState, self.callback_sensor_state)
    ...
    def sort_bump(self, msg):
        ...
        if msg.state:
            self.bumped = True
            self.stopped_until = rospy.Time.now() + rospy.Duration(secs=self.stopped_time)
	...
	Parameter laserscan is received from callback function.'''
        if rospy.Time.now() > self.stopped_until:
            self.stopped = False
            self.bumped = False
